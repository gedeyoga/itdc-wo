<template>
    <div>
        ...
        <div class="row mb-5">
            ...
            <div class="col-lg-4">
                <div class="card" style="height: 100%; max-height: 300px;" v-loading="loadingSummary">
                    <div
                    ...
                    <div class="card-body">
                        <canvas height="200" id="chartWo"></canvas>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="card" style="height: 100%; max-height: 300px;" v-loading="loadingSummary">
                    <div
                        class="card-header d-flex justify-content-between align-items-center"
                    >
                        <h5 class="m-0">Staffs</h5>
                    </div>
                    <div class="card-body overflow-auto">
                        <ul style="list-style: none; padding-left: 0;" >
                            <li v-for="(user , index) in summary.group_assignees" :key="index" class="border-bottom py-2 d-flex align-items-center">
                                <el-avatar :size="'small'" :src="user.default_profile"></el-avatar>
                                <span style="margin-left: 15px;">{{ user.name }}</span>
                            </li>

                            <li v-if="summary.group_assignees.length == 0">
                                <img class="w-50 d-block mx-auto" :src="require('../../../../images/empty.png')" alt="">
            ...
            <div class="col-lg-4">
                 <div class="card" style="height: 100%; max-height: 300px;" v-loading="loadingSummary">
                    ...
                    <div class="card-body overflow-auto">
                        <ul style="list-style: none; padding-left: 0;" >
                            <li v-for="(location , index) in summary.group_locations" :key="index" class="border-bottom py-2 d-flex align-items-center">
                                <span style="margin-left: 15px;">{{ location.name }}</span>
                            </li>
                            <li v-if="summary.group_locations.length == 0">
                                <img class="w-50 d-block mx-auto" :src="require('../../../../images/empty-location.png')" alt="">
                                <p class="text-center">Tidak ada data</p>
                            </li>
                        </ul>
                    </div>
            ...
            <div class="card-body">
                ...
                >
                    ...
                    <el-table-column width="220" prop="name" label="Workorder">
                        <template slot-scope="scope">
                           <a href="#" @click.prevent=" $refs['dialogWorkorderRef'].openDialog(scope.row.id)">{{ scope.row.name }}</a>
                        </template>
                    </el-table-column>
...
import DialogWorkorderForm from '../../Workorder/components/DialogWorkorderForm.vue';
...
export default {
    ...
    methods: {
        ...
        initChartJs() {
            ...
            const ctx = document.getElementById('chartWo');
        ...
        async fetchSummary() {
            ...
            try {
                const response = await axios.get(route('api.work-order.report.daily') , {
                    params: {
                ...
                if(response.status == 200) {
                    let group_status = {};
                    if(Array.isArray(response.data.group_status)) {
                        response.data.group_status.forEach((item , key) => {
                            group_status[item.status] = item.jumlah_status;
                        });
                        
                    }

                    this.summary.group_status = {...this.summary.group_status, ...group_status};
                    this.summary.group_assignees = [...response.data.group_assignees];
                    this.summary.group_locations = [...response.data.group_locations];

                    let statusAmount = [];
                    for (const key in this.summary.group_status) {
                        statusAmount.push(this.summary.group_status[key]);
                    }

                    console.log(statusAmount);

                    this.chart.data.datasets.forEach((dataset) => {
                        dataset.data = statusAmount;
                    });
